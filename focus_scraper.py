...
class FocusScraper():
    ...
    def get_sem_results(self, academic_year, semester, cache_results=True):
        ...
        if semester not in semesters:
            ...
            self.main_page = BeautifulSoup(response.content, 'html.parser')
            self.extract_body_from_frame()
                ...
                for a_tag in a_tags:
                    try:
                        ...
                        if self.COURSENAME_HREF in link:
                            course_name = str(a_tag.text).split("-")[0] \
                                              .strip()
            ...
            if len(courses_dict.keys()) == 0:
                ...
                #self.main_body = BeautifulSoup(response.content, 'html.parser')
    ...
    def monitor_semester(self, function, interval_seconds, args):
